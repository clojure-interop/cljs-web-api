...
(def constructor
  ...
  type
  A `web.dom.DOMString` containing the name of the event. This is always \\\"tonechange\\\".
  options
  A dictionary of type RTCDTMFToneChangeEventInit, which may contain one or more of the following fields:

  tone
  A `web.dom.DOMString` containing a single DTMF tone character which has just begun to play, or an empty string (\\\"\\\") to indicate that the previous tone has stopped playing. See Tone characters for details on what characters are permitted.
